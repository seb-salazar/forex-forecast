"""
MCS approach by Michael Gong - see https://michael-gong.com/blogs/model-confidence-set/
"""
import os
import numpy as np
...
ids = ["USDJPY", "USDCAD", "USDCHF", "EURUSD", "GBPUSD"]
models = ["ann", "arima", "arima-garch", "holt", "lstm", "ma"]

for id_ in ids:
    list_maes_per_currency = []
    for model in models:
        df = pd.read_csv(
            f"./MAEs/{id_}/{model}_{id_}.csv",
            names=["mae"],
            index_col=None, header=0
        )
        df.columns = [model]
        list_maes_per_currency.append(df)

    data = pd.concat(list_maes_per_currency, axis=1)

    mcs = ModelConfidenceSet(data, 0.05, 20, 1185).run()

    results_directory = "./results/"
    if not os.path.exists(results_directory):
        os.makedirs(results_directory)

    mcs.pvalues.to_csv(results_directory + f"{id_}_pvalues.csv", header=["p-values"])
